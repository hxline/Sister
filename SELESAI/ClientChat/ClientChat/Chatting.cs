...
namespace ClientChat
{
    public partial class Chatting : Telerik.WinControls.UI.RadForm
    {
        ...
        public Chatting(string loginname, string server)
        {
            InitializeComponent();
        ...
        public void Send()
        {
            ...
            try
            {
                ...
                if (radTextMessage.Text != null && !string.IsNullOrWhiteSpace(radTextMessage.Text))
                {
                    ...
                    string BoxTimer = comboBoxTimer.Text;
                    if (comboBoxTimer.Text != "")
                    {
                        //Multithread 2 fungsi
                        Task TCountdown = Task.Factory.StartNew(() =>
                            {
                                CountDown countdown = new CountDown(BoxTimer);
                                countdown.ShowDialog();
                            }
                            );

                        Task TSend = Task.Factory.StartNew(() =>
                            {
                                hub.Invoke("SendMessageToAll", Un, plaintext, BoxTimer).Wait();
                            }
                            );

                        Task.WaitAll(TCountdown, TSend);
                    }
                    else
                    {
                        hub.Invoke("SendMessageToAll", Un, plaintext, 0).Wait();
                    }
                }
                else
                {
                    hub.Invoke("SendMessageToAll", Un, radTextMessage.Text, 0).Wait();
                }
            }
            catch
            {
            ...
            try
            {
                ...
                var newUser = hub.On<string, string>("onNewUserConnected", (id, username) =>
                {
                    Sync.Post(delegate
                    {
                        ...
                        radListOnline.DataSource = ConnectedUsers;
                        radButtonSlide.BackColor = Color.Maroon;
                    }, null);
                });
                #endregion
        ...
        {
            radButtonSlide.BackColor = Color.Transparent;
            if (this.Width >= 594)
            {
