...
namespace ServerHost
{
    public class MyHub : Hub
    {
        ...
        public void SendMessageToAll(string userName, string message, int time = 0)
        {
            if (time != 0)
            {
                Console.WriteLine(userName + " akan mengirim pesan publik dalam waktu " + time + " detik");
                Timing(time);
            }
        ...
        private void AddMessageinCache(string userName, string message, int time)
        {
            CurrentMessage.Add(new MessageDetail { Index = CurrentMessage.Count , UserName = userName, Message = message, Time = time });
