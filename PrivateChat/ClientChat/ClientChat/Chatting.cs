...
namespace ClientChat
{
    public partial class Chatting : Telerik.WinControls.UI.RadForm
    {
        ...
        static BindingList<Members> userPrivate = new BindingList<Members>();
        ...
        public void Load_private_user(string conId = "")
        {
            var Sync = SynchronizationContext.Current;
            try
            {
                #region User baru online
                var newUser = hub.On<string, string>("GetUser", (id, username) =>
                {
                    Sync.Post(delegate
                    {
                        if (id != "")
                        {
                            userPrivate.Add(new Members(username, id));

                            radListUserPrivate.DataSource = userPrivate;
                        }
                    }, null);
                });
                #endregion

                hub.Invoke("SendPrivateMessage", conId).Wait();
            }
            catch
            {
                MessageBox.Show("Gagal terhubung ke server", "Gagal", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void radListOnline_DoubleClick(object sender, EventArgs e)
        {
            Members member = (Members)radListOnline.SelectedValue;
            Load_private_user(member.id);
        }
